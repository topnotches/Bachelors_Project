...
class XModelNodeElemAdd(XModelNodeElemOp):
    ...
    def infer_shape(self, layout: Layout) -> NoReturn:
        ...
        curr_seq_dims_in_a, curr_seq_dims_in_b = self.sequence_dims[self.layout]["in"]
        if len(curr_seq_dims_in_a) > 1 and len(curr_seq_dims_in_b) > 1:
            assert (
                curr_seq_dims_in_a == curr_seq_dims_in_b
            ), f"[ERROR] Mis-matched dimension sequences of inputs (xnnc ElemAdd): {curr_seq_dims_in_a}, {curr_seq_dims_in_b}. Op name: {self.op_name}."

        # compute outputs_tensor
        a, b = self.inputs_tensor
        c = XTensorCompute.elem_add(a, b)
...
class XModelNodeElemMul(XModelNodeElemOp):
    ...
    def infer_shape(self, layout: Layout) -> NoReturn:
        ...
        assert (
            self.inputs_tensor is not None and len(self.inputs_tensor) == 2
        ), f"[ERROR] xnnc ElemMul requires two inputs: actual: {len(self.inputs_tensor)}. op name: {self.op_name}"
        assert all(
            [
                layout.name == in_tensor.data_format.name
                for in_tensor in self.inputs_tensor
        ...
        if len(curr_seq_dims_in_a) > 1 and len(curr_seq_dims_in_b) > 1:
            assert (
                curr_seq_dims_in_a == curr_seq_dims_in_b
            ), f"[ERROR] Mis-matched dimension sequences of inputs (xnnc ElemMul): {curr_seq_dims_in_a}, {curr_seq_dims_in_b}. Op name: {self.op_name}."

        # compute outputs_tensor
        a, b = self.inputs_tensor
        c = XTensorCompute.elem_mul(a, b)
        self.outputs_tensor = [c]
...
class XModelNodeConcat(XModelNode):
    ...
    def infer_shape(self, layout: Layout) -> NoReturn:
        ...
        assert all(
            [
                self.sequence_dims[self.layout]["in"][0] == seq
                for seq in self.sequence_dims[self.layout]["in"]
            ]
        ), f"[ERROR] mis-matched dimension sequences of input (xnnc Concat): {self.sequence_dims[self.layout]['in']}. Op name: {self.op_name}."
...
class XModelNodeSqueeze(XModelNode):
    ...
    def infer_shape(self, layout: Layout) -> NoReturn:
        ...
        ), f"[ERROR] xnnc Squeeze requires the rank of input tensor is 4: actual: {self.inputs_tensor[0].ndims}. op name: {self.op_name}"
        assert (
            self.inputs_tensor[0].data_format.name == layout.name
...
class XModelNodeCustomSampling(XModelNode):
    """
    Custom Sampling tf2 keras layer used in VAE.
    ...
    def __init__(self, op_name: str):
        assert op_name is not None and isinstance(
            op_name, str
        ), "'op_name' should not a str value."
        XModelNode.__init__(self, op_name, "sampling")
    ...
    def infer_shape(self, layout: Layout) -> NoReturn:
        ...
        assert (
            self.inputs_tensor is not None and len(self.inputs_tensor) == 2
        ), f"[ERROR] xnnc {self.op_type} requires two inputs: actual: {len(self.inputs_tensor)}."
        assert (
            self.inputs_tensor[0].ndims >= 2
        ), f"[ERROR] the dimension of the primary input tensor should be 2 or more."
        assert all(
            [
                layout.name == in_tensor.data_format.name
                for in_tensor in self.inputs_tensor
    ...
    def deserialize(node: openir.NodeProto) -> "XModelNode":
        raise NotImplementedError(
            "Not implement deserialize interface of XModelNodeCustomSampling"
        )
